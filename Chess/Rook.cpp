...
Rook::Rook(char type, char color, Position pos) : Solider(type, color, pos)
{
}
...
int Rook::checkMove(Position* to, Board* gameBoard, char turn)
{
    ...
    {
        if (gameBoard->isClearVertical(&pos, to) || gameBoard->isClearHorizontal(&pos, to))
        {
            return V_MOVE;
        }
    }
