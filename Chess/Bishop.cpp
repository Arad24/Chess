...
Bishop::Bishop(char type, char color, Position pos) : Solider(type, color, pos)
{
}
...
int Bishop::checkMove(Position* to, Board* gameBoard, char turn)
{
    ...
    if (gameBoard->getByPlace(to->getX() - 97, to->getY())) // If in destination exist solider
    {
    ...
    {
        if (gameBoard->isClearDiagonal(&pos, to))
        {
            return V_MOVE;
        }
    }
